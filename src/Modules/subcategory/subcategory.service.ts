...
export class SubcategoryService {
    ...
    async updateSubcategory(user: UserDocument, subcategoryId: Types.ObjectId, body?: updateSubcategoryDto, file?: Express.Multer.File):
    ...
    async findById(id: Types.ObjectId, populate?: PopulateOptions[]): Promise<SubcategoryDocument> {
    ...
    async findAll(query?: SubcategoryQueryDto): Promise<ISubcategory[] | IPaginate<ISubcategory>> {
        ...
        return this.subcategoryRepository.find({ filter, populate: query?.populate, select: query?.select, sort: query?.sort, page: query?.page, limit: query?.limit });
    }

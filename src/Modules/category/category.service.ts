...
export class CategoryService {
    ...
    async updateCategory(user: UserDocument, categoryId: string, body?: updateCategoryDto, file?: Express.Multer.File):
    ...
    async findById(id: string, populate?: PopulateOptions[]): Promise<CategoryDocument> {
    ...
    async findAll(query?: CategoryQueryDto): Promise<ICategory[] | IPaginate<ICategory>> {
        ...
        return this.categoryRepositoryService.find({ filter, select: query?.select, sort: query?.sort, page: query?.page, limit: query?.limit });
    }

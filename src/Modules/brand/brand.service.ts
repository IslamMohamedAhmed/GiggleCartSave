...
export class BrandService {
    ...
    async updateBrand(user: UserDocument, brandId: Types.ObjectId, body?: updateBrandDto, file?: Express.Multer.File):
    ...
    async findById(id: Types.ObjectId, populate?: PopulateOptions[]): Promise<BrandDocument> {
    ...
    async findAll(query?: BrandQueryDto): Promise<IBrand[] | IPaginate<IBrand>> {
        ...
        return this.brandRepository.find({ filter, populate: query?.populate, select: query?.select, sort: query?.sort, page: query?.page, limit: query?.limit });
    }
